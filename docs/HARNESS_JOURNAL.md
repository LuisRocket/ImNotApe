...
## Stage 9 — Funnel 단순화: 산업 단계 제거 + 회사 풀 5→8 (2026-05-08)

### 컨텍스트
사용자 제기 — *"섹터 부분이 비슷비슷해서 헷갈리기만 하고, 오히려 회사를 선택하는 부분에서는 힌트로 작용하는 문제가 있어. 섹터를 맞추는 부분을 없애고, 대신 회사 선택지의 풀을 늘리는건 어떨까."*

진단을 풀어보면:
- 산업 7지선다는 인접 산업("Online Brokerage Mobile-First" vs "Online Brokerage Traditional" vs "Crypto Exchange")이 noise만 만들고 *변별력이 약함*
- 더 큰 문제: 산업 7개를 **먼저** 보는 것이 회사 funnel에 *암묵 단서*로 작용. 플레이어가 산업을 고르고 나면 회사 5지선다가 사실상 1~2개로 좁혀짐
- catalog의 `scoring.industry_distance`는 어차피 빈 객체라 산업 점수는 거의 0이었음. 의도된 0~1000 점수 구간이 실제로 동작하지 않음 — 잘 모르고 두 단계 funnel을 *유지*하고 있던 것

### 결정
1. **산업 funnel 제거** (UI에서 step 1을 통째로 제거, 회사 단계만 유지)
2. **회사 풀 5 → 8개로 확장** — 단일 척도 단순화에 따른 난이도 보상
3. **점수: 회사 정답 = 5,000 만점 (단일)** — 기존 4000(회사) + 0~1000(산업) 분리 폐기
4. **catalog 스키마 후방호환** — `industry_options`, `scoring.industry_distance` 필드는 그대로 두고 frontend가 안 쓰면 됨. 새 카탈로그 빌드 시 제거 예정.

### 빌드한 것
- `src/routes/+page.svelte` — `pickedIndustry`/`industryDistance` 제거, 점수 공식 단일화, result-card 산업 row 제거 + 정답 회사 표시 추가
- `scripts/expand-company-pool.mjs` — 30개 catalog × 미끼 3개 hand-curated 인라인. 멱등(이미 8개면 skip).
- 미끼 큐레이션 원칙: 같은 산업 우선 + 인접 산업 1~2개 섞어 변별력 유지, 정답 시기에 존재한 회사
...
| 풀 크기 | 5 | 8 |
| 정답 확률 (랜덤) | 20% | 12.5% |
| 점수 척도 수 | 2 (산업+회사) | 1 (회사) |
| 만점 | 5,000 | 5,000 |
| 힌트 사용 후 floor | 1,500 | 1,500 |

### 교훈
1. **"옵션이 많으면 어렵다"는 직관은 funnel 구조에서 깨짐**. 산업 → 회사 순서로 두면 산업 옵션 자체가 *회사 단계의 단서*로 새어나간다. 추리 게임에서 *선택 순서*는 정보 leakage 방향을 결정함. 단일 척도가 깔끔.
2. **score가 사실상 안 움직이고 있던 걸 일찍 못 잡았음**. `scoring.industry_distance`가 빈 객체라 산업 점수 = 0인데도 UI에 0~1000 표시가 있었음. *측정 코드*가 콘텐츠 layer와 분리돼서 silently 0이 나옴. UI 컴포넌트 단위로 *최소 1번 정상값을 보는* 시각적 health check가 필요.
3. **catalog 스키마 후방호환을 그대로 두는 결정**. `industry_options` 필드를 지금 다 지우면 다음 batch에서 다시 채우는 작업·에이전트 가이드 수정으로 cascading 변경 큼. *프런트엔드가 안 쓰면 죽은 필드*로 남겨두고 다음 카탈로그 빌드(30→100~300) 시 한 번에 정리하는 게 비용/리스크 균형.
4. **풀 8개의 미끼 큐레이션 시간**: 30 catalog × 3 미끼 = 90개를 hand-curated 인라인으로 5분 안에 작성. 여기서 *작가/큐레이터 batch 모드에 맡기지 않고 직접 한 결정*은 옳았음 — 큐레이션 사고 자체가 단순(같은 산업 + 시기·규모 인접)해서 LLM 호출 비용이 작업 비용보다 큼. 게임 디자인에 *moderately structured* 작업은 코드 인라인이 효율적.
...
scripts/
  ...
  expand-company-pool.mjs      ← Stage 9: 회사 풀 5→8 backfill (다음 batch부턴 큐레이터가 8개로 출력)
...
11. **catalog 스키마 청소 시점**: Stage 9에서 `industry_options`/`scoring.industry_distance`를 dead field로 둠. 다음 카탈로그 빌드(목표 100~300)와 함께 schema 정리 + 큐레이터 batch 모드 가이드에 *회사 풀 8개* 명시 + 산업 funnel 출력 제거가 한 번에 되어야 함.
