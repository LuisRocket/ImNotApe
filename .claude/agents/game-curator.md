...
**입력**: `_workspace/catalog/{batch-id}/01_analyst-enriched.json` (분석가의 120개)

**작업이 daily 모드와 다른 점**:

1. **다양성 강제 (가장 중요)**: 120개를 상호 비교하면서 카탈로그 전체의 균형을 맞춘다.
   - 산업 분포: 같은 산업 5개 이상 안 됨. 분배 가이드 (예시):
     - Tech/SaaS: 15
     - Semiconductors: 5~7
     - Retail: 10
     - Healthcare: 8~10
     ...
     - Auto/Materials: 5~7
     - 기타: 채움
   - 시대 분포: 2016~2025 균형. 한 해에 30개 이상 몰리지 않게. COVID(2020)와 AI붐(2023~24)은 자연스럽게 더 많을 수 있음 (각 ~15%까지 OK).
   - 난이도 분포: easy 20% / medium 60% / hard 20%.

2. **상호 비교 기반 유니크성 검증**:
   - 같은 산업에서 비슷한 재무 지문 회사가 둘 있으면 *더 distinctive한 1개만 채택*. 예: COST FY2024와 COST FY2023 둘 다 후보면 special dividend 있는 FY2024만.
   - 미끼 풀 설계 시 *카탈로그 내 다른 회사*를 미끼로 활용 가능 (다른 challenge의 정답이 되는 회사가 미끼로 등장하는 건 OK — 사용자가 다른 challenge에서 그 회사를 만났을 때 학습 효과).

3. **120개 → 100개 추림**: 다양성 + 유니크성 + 흥미도 종합 평가.

4. **각 승인 후보별 funnel 설계**: 산업 옵션 7개, 회사 풀 5개 (정답+미끼 4), scoring_hints의 industry_distance.

**출력**: `_workspace/catalog/{batch-id}/02_curated.json` — 100개의 승인된 challenge + 각각의 funnel·미끼·scoring_hints

**거절 루프 최소화**: daily 모드와 달리 분석가에게 재요청하지 않음 (120개 풀이 충분히 큼). 정 부족하면 오케스트레이터가 분석가를 재호출.
